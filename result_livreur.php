...
<center><h1> Informations pour le livreur</h1><br/>
	...
	$query="SELECT Route.Nom,Route.Type,Route.Etat, COUNT(Client.num_client) AS nb_client FROM Jonction 
			...
			INNER JOIN Route ON Route.Nom=Liaison.nom_r AND Route.Type=Liaison.type_r --Pour recup l'etat
			INNER JOIN Habitation ON Habitation.nom_route=Liaison.nom_r AND Habitation.type_route=Liaison.type_r --On chope le client
